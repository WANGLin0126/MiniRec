...
class AbstractPrompter:
    ...
    def __init__(self,
        ...
        if hasattr(self.tokenizer, 'generation_prompt'):
            self.generations = [self.tokenizer.encode(self.tokenizer.generation_prompt,
                                                      add_special_tokens=False),
                                self.tokenizer.encode(self.tokenizer.generation_end,
                                                      add_special_tokens=False)]
        else:
            assert not self.add_completion_range, "If add_completion_range is True, tokenizer must have generation_prompt"
    ...
    def formatting_func(self, examples,
        ...
        if isinstance(examples[prompt_key], list):
            ...
            for i in range(len(examples[prompt_key])):
                ...
                if sys_prompt_key is not None:
                    conversation += [{"role": "system",
                                      "content": examples[sys_prompt_key][i]}]

                conversation += [{"role": "user",
                                  "content": examples[prompt_key][i]}]

                if completions_key is not None:
                    conversation += [{"role": "assistant",
                                      "content": examples[completions_key][i]}]
                if not return_messages:
                    if "deepseek" in self.tokenizer.name_or_path.lower():
                        ...
                        conversation = conversation.replace("<｜end▁of▁sentence｜>", "")
            ...
            if sys_prompt_key is not None:
                conversation += [{"role": "system",
                                  "content": examples[sys_prompt_key]}]

            conversation += [{"role": "user", "content": examples[prompt_key]}]

            if completions_key is not None:
                conversation += [{"role": "assistant",
                                  "content": examples[completions_key]}]

            # if not isinstance(conversation[0], list):
    ...
    def totensor(self, element, max_length=4096, **kwargs):
        ...
        if self.assistant_content_key is not None and self.add_completion_range:
            result |= {
                "completion_range": self.find_completion_start_end(outputs["input_ids"])
            }
        return result
