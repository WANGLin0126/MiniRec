...
log_filename = os.path.join(log_dir, time.strftime("train_%Y%m%d_%H%M%S.log"))

# === 创建 logger ===
logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)  # 设置为 DEBUG 级别

# === 创建 FileHandler（保存到文件） ===
file_handler = logging.FileHandler(log_filename, mode='a', encoding='utf-8')
file_handler.setLevel(logging.DEBUG)

# === 创建 Formatter ===
formatter = logging.Formatter(
    "%(asctime)s [%(levelname)s] GPU-%(gpu)d: %(message)s"
)
...
# === 创建 LoggerAdapter，自动注入当前 GPU ID ===
class GPUAdapter(logging.LoggerAdapter):
    def process(self, msg, kwargs):
        gpu_id = torch.cuda.current_device() if torch.cuda.is_available() else -1
        return msg, {"extra": {"gpu": gpu_id}}

# === 包装 logger ===
gpu_logger = GPUAdapter(logger, {})
...
class GenRecTrainer(Trainer):
    ...
    def __init__(
        ...
        for split in ['train', 'valid', 'test']:
            ...
            # print("=*"*100)
            # print(full_dataset[split][0].keys())
    ...
    def _generate_item_embeddings(self, model,
        ...
        with torch.no_grad():
            ...
            Item_id = []
            Item_h = []
            for batch in all_item_hs:
                c = c + len(batch['item_h'])
                d = d + len(batch['item_id'])
                Item_h.append(batch['item_h'])
                Item_id.append(torch.tensor(batch['item_id']))
                # print(batch['item_id'].device)
            
              
            # min_id = min(np.unique(Item_id))
            # max_id = max(np.unique(Item_id))
            # len_unique = len(np.unique(Item_id))
            # gpu_logger.debug(f"Befor gather, batches in all_item_hs: {len(all_item_hs)}, Total: {d}, Len Unique: {len_unique} Min_id: {min_id}, Max_id: {max_id}")

            Item_h = torch.cat(Item_h, dim=0)
            Item_id = torch.cat(Item_id)
            Item_id = Item_id.to(torch.cuda.current_device())
            gpu_logger.debug(f"Item_h shape: {Item_h.shape}, Item_id len: {len(Item_id)}")
            ...
            if self.accelerator.num_processes > 1:
                ...
                Item_h_ga = [torch.zeros_like(Item_h) for i in range(torch.distributed.get_world_size())]
                Item_id_ga = [torch.zeros_like(Item_id) for i in range(torch.distributed.get_world_size())]
                torch.distributed.all_gather(Item_h_ga, Item_h, group=None, async_op=False)
                torch.distributed.all_gather(Item_id_ga, Item_id, group=None, async_op=False)
                
                Item_h_ga = torch.cat(Item_h_ga, 0)
                Item_id_ga = torch.cat(Item_id_ga, 0)
                
                # all_item_hs = self.accelerator.gather_for_metrics(all_item_hs)
                # Item_h_ga = self.accelerator.gather_for_metrics(Item_h)
                # Item_id_ga = self.accelerator.gather_for_metrics(Item_id)
                
            else:
                Item_h_ga = Item_h
                Item_id_ga = Item_id
            
            gpu_logger.debug(f"Item_h_ga shape: {Item_h_ga.shape}, {Item_h_ga[0].shape} Item_id_ga shape: {Item_id_ga.shape}")
                
            for id, h in zip(Item_id_ga, Item_h_ga):
                item_hs[id.item()] = h
            
            print(f"item_hs[0]: {item_hs[0]}")
            gpu_logger.debug(f"item_hs2 length: {len(item_hs)}")
            ...
            #     item_ids = item_batch_dict["item_id"]
            #     item_h   = item_batch_dict["item_h"]
            #     # gpu_logger.debug(f"the length of item_batch_dict item_h: {len(item_h)}, item_ids: {len(item_ids)}, Max item ID: {max(item_ids)}")
            ...
            #         # print(len(item_hs), end = " ")
            #     bbb = bbb + len(item_batch_dict["item_id"])
            #     min_id_ = min(item_batch_dict["item_id"])
            #     max_id_ = max(item_batch_dict["item_id"])
            #     gpu_logger.debug(f"{aaa}/{len(all_item_hs)} batchs, The item_hs ID keys: {len(list(item_hs.keys()))}, Min ID: {min_id_}, Max ID: {max_id_}, Counted item_h: {bbb} ")
            #     aaa = aaa + 1
            
            gpu_logger.debug(f"The gathered length of item_hs: {len(item_hs)}")
            
            self.accelerator.wait_for_everyone()
            
            
            if len(item_hs) != len(self.item_dataset):
                for item_batch_dict in all_item_hs:
                    item_id_ = item_batch_dict["item_id"]
                    item_h_  = item_batch_dict["item_h"]
                    len_ = len(item_batch_dict["item_id"])
                    gpu_logger.debug(f"item_batch_dict: {item_batch_dict}")
                    gpu_logger.debug(f"Length of item id: {len_}")
                    gpu_logger.debug(f"item_batch_dict[item_id]: {item_id_}")
                    gpu_logger.debug(f"item_batch_dict[item_h]: {item_h_}")
                    for batch_idx in range(len(item_batch_dict["item_id"])):
                        item_id = item_batch_dict["item_id"][batch_idx]
                        item_h = item_batch_dict["item_h"][batch_idx]
                        item_hs[item_id] = item_h.to(self.accelerator.device)
            ...
            assert len(item_hs) == len(
                self.item_dataset), f"{len(item_hs)} != {len(self.item_dataset)}"
    ...
    def evaluate(self,
        ...
        #     warnings.warn("The eval_dataset parameter is provided but will be ignored. ")
        # print('-'*80)
        # if eval_dataset is not None:
        #     print("GRecTrainer: eval_dataset is not None")
        #     # print(eval_dataset)
        # elif eval_dataset is None:
        #     print("GRecTrainer: eval_dataset is None")
        #     # print(self.eval_dataset.keys())
        # print('-'*80)
        ...
        with torch.no_grad():
            ...
            # calculate profile avg length
                ...
                eval_output = super().evaluate(
                    # TODO: eval_dataset=self.eval_dataset[_set_name]
                    eval_dataset=self.eval_dataset[_set_name],
    ...
    def _generate_in_train(self, model, batch,):
        ...
        if self.args.use_vllm:
            ...
            # tokenizer = get_tokenizer('/storage_fast/lwang/SeqRecDistill/llm_models/Qwen2.5-3B-Instruct')


            gpu_logger.debug(f"Before gather-len(user_results): {len(user_results)}")
                # distribute back to each device
            # print(f"Before gather-len(user_results): {len(user_results)}")
            
            user_results = self.accelerator.gather_for_metrics(
                [user_results], use_gather_object=True)[0]
            gpu_logger.debug(f"After gather-len(user_results): {len(user_results)}")
            # print(f"After gather-len(user_results): {len(user_results)}")
            

            # 简化断言，只打印关键变量
            if len(user_results) != full_batch_size * num_processes:
                gpu_logger.error(f"Assertion failed: len(user_results) != full_batch_size * num_processes")
                gpu_logger.error(f"  - len(user_results): {len(user_results)}")
                gpu_logger.error(f"  - full_batch_size: {full_batch_size}")
                gpu_logger.error(f"  - num_processes: {num_processes}")
                gpu_logger.error(f"  - expected: {full_batch_size * num_processes}")
            
            assert len(user_results) == full_batch_size * num_processes, f"{len(user_results)} != {full_batch_size} * {num_processes}"
    ...
    def _augment_with_reasoning(self, train_data_id, reasoning):
        element = self.train_dataset[train_data_id] | {
            # TODO: the "reasoning": reasoning is not correct, it should be the "profile"
            "profile": reasoning}
    ...
    def log(self, logs: Dict[str, float], *args, **kwargs) -> None:
        ...
        for metric_key_prefix in list(self._stored_metrics.keys()):
