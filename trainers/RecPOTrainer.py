...
class RecPOTrainer(GenRecTrainer):
    ...
    def get_train_dataloader(self):
        """
        Return the training dataloader for RecPOTrainer
        根据 self.shuffle 决定是否打乱
        """
        train_dataset = self.train_dataset
        if train_dataset is None:
            raise ValueError("RecPOTrainer: training requires a train_dataset.")

        # 单卡 = RandomSampler / SequentialSampler
        if self.args.local_rank == -1:
            sampler = RandomSampler(train_dataset) if self.shuffle else SequentialSampler(train_dataset)
        else:  # 多卡 = DistributedSampler
            sampler = DistributedSampler(train_dataset, shuffle=self.shuffle)
    ...
    def asymmetric_tanh_torch(self, A, alpha=2.5, beta=1.1, gamma=2.0):
        """
        PyTorch 版本，支持任意维度 Tensor
        """
        tanh_val = torch.tanh(beta * A)
        return torch.where(A > 0, alpha * tanh_val, gamma * tanh_val)
    ...
    def compute_rec_score(self,
        ...
        if self.args.advantage_type == 'gaussian':
            if num_samples == 1:
                ...
                # advantages = self.asymmetric_tanh_torch(advantages) # (B, num_samples)
    ...
    def batch_forward(self, model, batch, prefix):
        ...
        self.last_token_hidden_states = last_hidden_states
